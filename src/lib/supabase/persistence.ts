...
export function createPersistence(client: SupabaseClient) {
  ...
  return { memoryStore, permissionStore: permissionStoreForUser(''), permissionStoreForUser,
    ...
    async searchChunks(userId:string,query:string,limit=8) { const {data,error}=await client.from('file_chunks').select('content,file_id,files!inner(owner_id)').eq('files.owner_id',userId).ilike('content',`%${query.slice(0,80).replace(/[%_]/g,'')}%`).limit(limit); if(error) return []; return (data??[]).map((r:any)=>({content:r.content,fileId:r.file_id})) },
